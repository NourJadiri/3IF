...
Ensemble_old::Ensemble_old(unsigned int card_max) {
...
/*Ensemble_old::Ensemble_old(int tab[], unsigned int size) {
    this->card_max = size;
    this->card_actuelle = 0;
...
Ensemble_old::Ensemble_old(const Ensemble_old & e){
...
#ifdef MAP
    cout << "Appel au constructeur de copie de <Ensemble_old>" << endl;
#endif
}*/

void Ensemble_old::Afficher(){
...
/*bool Ensemble_old::estEgal(const Ensemble_old &e) const {
    bool estEgal = true;
...
unsigned int Ensemble_old::estInclus(const Ensemble_old &e) const {
...
Ensemble_old::~Ensemble_old(){
    delete[] elements;
    ///delete this : ON NE DELETE JAMAIS THIS PCQ ON NA PAS ACTIVEMENT ALLOCATE THIS

#ifdef MAP
    cout << "Appel au destructeur de <Ensemble_old>" << endl;
#endif
}

/*
bool Ensemble_old::contains(int value) {
    int i;
    for(i = 0 ; i < card_actuelle ; i++)
