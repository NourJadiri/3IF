...
int computeD2(int i, set s, int n, int** cost, int** memD){
    c++;
    // Preconditions: isIn(i,s) = false and isIn(0,s) = false
    // Postrelation: return the cost of the smallest path that starts from i, visits each vertex of s exactly once, and ends on 0
    if(memD[i][s] > 0){
    ...
    for (int j=1; j<n; j++){
        if (isIn(j,s)){
            int d = computeD2(j, removeElement(s,j), n, cost, memD);
            if (cost[i][j] + d < min) min = cost[i][j] + d;
         }
    }
    memD[i][s] = min;
...
int main(){
    ...
    int ** memD = (int**)malloc(n*sizeof(int*));
    for(int i = 0 ; i < n ; i++){
        memD[i] = (int*)malloc((1<<n)*sizeof(int));
    }
    ...
    clock_t t = clock();
    int d = computeD2(0, s, n, cost, memD);
