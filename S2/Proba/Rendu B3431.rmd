...
```{r}
knitr::kable(resultsMM2, format = "markdown", caption = "File M/M/2")
```
Pour de faibles valeurs de $\lambda$, nous ne remarquons pas de différence significative. Cepedant, plus $\lambda$ augmente, plus on voit que le nombre de client moyen, ainsi que le temps moyen d'attente diminue (d'un facteur 2 environ), ce qui est en accord avec notre intuition : <i> deux serveurs en même temps feront avancer la file deux fois plus vite </i>