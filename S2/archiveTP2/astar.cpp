    ...
    while (!q.empty()){
        ...
        for (int i=0; i<nbActions; i++){
            ...
            if (map.count(ss)==0 || map[s].g + g.getCost(s, i) < map[ss].g){
                ...
                map[ss].pred = s;
                q.push( {ss, map[ss].g + g.heuristic3(ss)} );
            }
        }
        map[s].isGrey = false;
    }
...
void astarWeighted(const State &s0, StateGraph &g, double w){
    // Search for the shortest path to go from s0 to a final state in g
    clock_t start = clock(); // start time used to compute CPU time
    int nbIter = 0;
    HashMap map;
...
void astarWeightedAnyTime(const State &s0, StateGraph &g, double w){
    // Search for the shortest path to go from s0 to a final state in g
    clock_t start = clock(); // start time used to compute CPU time
    int nbIter = 0;
    HashMap map;
...
int main(){
    StateGraph g;
    astarWeightedAnyTime(g.initialState(), g, 1.5);
    return 0;
}
