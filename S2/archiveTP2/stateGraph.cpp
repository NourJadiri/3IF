...
int StateGraph::heuristic1(const State &s) const{
    // return a lower bound of the length of the shortest path from s to a final state
    //int c = s.getNbBlocsTotal() - s.getNbBlocs(s.getStackNumber()-1);
    // Insert your code here to implement a more informed heuristic!
    return nbBlocs - s.getNbBlocs(nbStacks-1);
}
...
int StateGraph::heuristic2(const State &s) const{
    ...
    int c = nbBlocs - s.getNbBlocs(lastStackIndex);
    int lastStackNbBlocs = s.getNbBlocs(lastStackIndex);
 
    for (int i = 0; i < lastStackNbBlocs; i++)
    {
        if(s.getBloc(lastStackIndex,i) != 'a'+nbBlocs-i-1)
        {
            return c + 2*(lastStackNbBlocs-i);
        }
    }
...
int StateGraph::heuristic3(const State &s) const{
    ...
    int d = 0; //les blocs quand on change de sens de variation
